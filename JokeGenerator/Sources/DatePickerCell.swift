    ...
    */
    ...
    open var date:Date = Date() {
        didSet {
            ...
            datePicker.locale = Locale(identifier: "bb")
            DatePickerCell.dateFormatter.locale = Locale(identifier: "bb")
            DatePickerCell.dateFormatter.dateFormat = "HH:mm"
...
//    }
    ...
    */
    ...
    @objc func datePicked() {
        ...
        let parseTime = formatter.string(from: date).split(separator: ":")
        SettingsInteractor().customTime.set(parseTime[0], forKey: UserDefaultKey.hour.rawValue)
        SettingsInteractor().customTime.set(parseTime[1], forKey: UserDefaultKey.minute.rawValue)
        print("iste feraset iste adamlik \(parseTime[0])  ,  \(parseTime[1])")
    }
}
